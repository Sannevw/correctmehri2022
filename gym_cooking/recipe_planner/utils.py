...
class Baked(Predicate):
    def __init__(self, obj):
        Predicate.__init__(self, 'Baked', (obj,))
        
class ChoppedWashed(Predicate):
    def __init__(self, obj):
        Predicate.__init__(self, 'ChoppedWashed', (obj,))
...
Post: Mixed(X-Y), !SomeState(X), !SomeState(Y)
'''
...
class Mix(Action):
    def __init__(self, arg1, arg2, pre=None, post_add=None):
        ...
        self.pre_default = [Fresh(arg1), Mixed(arg2)]
        self.post_add_default = [Mixed('-'.join(sorted(arg1.split('-') + arg2.split('-'))))]

        Action.__init__(self, 'Mix', pre, post_add)
...
class Wash(Action):
    def __init__(self, obj, pre=None, post_add=None):
        ...
        self.pre_default = [NoPredicate()] # !TODO chopped and fresh..
        self.post_add_default = [ChoppedWashed(obj)]

        Action.__init__(self, 'Wash', pre, post_add)
...
class Bake(Action):
    def __init__(self, obj, pre=None, post_add=None):
        ...
        self.pre_default = [Fresh(obj)]
        self.post_add_default = [Baked(obj)]
